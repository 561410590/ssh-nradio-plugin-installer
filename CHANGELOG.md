...
## V3.2.0 - 2026-09-14（本地更新）

- `5 > 5 > 2` 加入 IPv4/IPv6 多线加权分流、双栈 IP/端口固定出口、IPv6 NAT66 和连接数、线路速率查看。
- 后台跟踪已有 WAN 线路上下线；C5800-688 双线路双栈访问已验证。
- 本地工作流接入 15 项分流测试，同步脚本与网页版本、配套包、下载路由和文件校验清单。

## NRadio Mesh 1.20.1-4 - 2026-09-14（本地下载包更新）

- 支持页与固定文件 `nradio-mesh.ipk` 更新为用户提供的 1.20.1-4 安装包。
- 工作流核对网页下载名称、包内版本、程序版本、文件权限和 Shell 语法。

## AK68-798 透传插件 1.2.0-5 - 2026-09-14（本地下载包新增）

- 新增支持页介绍及本地下载，安装在 C5800（NROS 2.2.15），管理 AK68-798 透传、恢复路由与状态查看。
- 新增下载路由，以及包名、版本、权限、配置格式和 Shell / JavaScript 语法检查。

## V3.1.0 - 2026-09-12（正式发布）

- `5 > 11` 新增硬件加速管理。
- 优化应用商店应用栏、打开按钮与操作弹窗，修复标签文字重叠；首页 CPU / 5G 切换按钮仅显示名称。
- 开启使用原厂 HNAT，关闭切回软件卸载，持久保存配置并启用原厂开机服务。
- 分开核对配置、HNAT 运行开关、IPv4/IPv6 卸载规则及硬件绑定条目；日志型状态读取只采用本次新增输出。
- 专用配置备份、切换失败恢复、重复操作跳过与 OpenClash 进程/规则检查。
- 修复内核日志滚动导致 HNAT 开关读取失败，补齐显式无备份切换模式。
- 首页温度组件 `20260912-6` 集成即时网速采样，按真实采样间隔计算，重复后台样本及页面刷新不清零，支持双线路独立统计。
- MT5700 双线路适配 `20260912-2` 转发各线路主动速率上报，修复重复采样和停流后旧速率残留；页面暂停互不影响，重连后恢复采样。
- 本地工作流同步最新组件版本，移除已删除测试文件的引用，直接验证 HNAT 日志读取与内嵌页面语法；更新脚本 SHA-256 和大小。
