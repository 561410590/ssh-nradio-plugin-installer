...
- 修复状态文件 Shell 单引号转义错误，阻止 OpenVPN / EasyTier 状态加载时的命令注入。
- 状态文件新增格式标记，拒绝加载旧版无标记状态文件并触发安全重建。
- 主线、beta 与临时安装器恢复 TLS 证书校验，移除不安全下载参数。
- 奇游和雷神临时安装器补入固定 SHA256 校验，校验失败拒绝执行远程脚本。
- 关键安装链在 `opkg update` 失败时停止，不再使用可能过期或不完整的索引继续安装。
- 仓库 CI 新增 Shell 语法、安全下载参数、状态值转义和注入载荷回归检查。
